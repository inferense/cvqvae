...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels, num_hiddens, num_residual_layers, num_residual_hiddens, stride):
        ...
        if stride == 4:
            stack.extend([
                nn.ConvTranspose2d(in_channels=num_hiddens, out_channels=num_hiddens // 2, kernel_size=4, stride=2,
                                   padding=1),
                nn.ReLU(True),
                nn.ConvTranspose2d(in_channels=num_hiddens // 2, out_channels=out_channels, kernel_size=4, stride=2,
                                   padding=1),
            ])

        elif stride == 2:
            stack.append(
                nn.ConvTranspose2d(in_channels=num_hiddens, out_channels=out_channels, kernel_size=4, stride=2,
                                   padding=1)
            )
...
class VQVAE(nn.Module):
    def __init__(self, in_channels, num_hiddens, num_residual_layers, num_residual_hiddens, num_embeddings,
        ...
        self.decode_top = Decoder(embedding_dim, embedding_dim, num_hiddens, num_residual_layers, num_residual_hiddens,
                                  stride=2)
        ...
        self.decode_ = Decoder(embedding_dim + embedding_dim, in_channels, num_hiddens, num_residual_layers,
                               num_residual_hiddens, stride=4)
